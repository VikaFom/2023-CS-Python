...
class Ocean:
    ...
    def __eq__(self, __value: "Ocean") -> bool:
    ...
    def gen_next_quantum(self) -> "Ocean":
        ...
        for i in range(len(self.state)):
            ...
            for j in range(len(self.state[i])):
                next_state[i].append(self.gen_next_cell(i, j))
        ...
        for ofset_i in range(-1, 2):
            for ofset_j in range(-1, 2):
                if ofset_i != 0 or ofset_j != 0:
                    cell_i = i + ofset_i
                    cell_j = j + ofset_j
                    if cell_i >= 0 and cell_i <= len(self.state) - 1 and cell_j >= 0 and cell_j <= len(self.state[i]) - 1:
                        result.append(self.state[cell_i][cell_j])
        return result
